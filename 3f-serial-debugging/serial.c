...
static int serial_probe(struct platform_device *pdev)
{
	...
	/*
	 * Debugfs inits
	 * Do them at the end, to avoid having to remove them in case of failure
	 * No need to test return value the debugfs_create_dir(), most drivers
	 * don't:
	 * - If CONFIG_DEBUG_FS is not set, the other debugfs functions expand
	 *   to empty code, so the return value from debugfs_create_dir will be
	 *   ignored.
	 * - Otherwise, the subsequent debugfs functions check IS_ERROR(parent)
	 *   and return without doing anything in case the creation of the
	 *   parent failed.
	 */
	serial->debugfs_dir = debugfs_create_dir(serial->miscdev.name, NULL);
	debugfs_create_atomic_t("counter", 0644, serial->debugfs_dir,
				&serial->counter);
...
static int serial_remove(struct platform_device *pdev)
{
	...
	pr_info("Called %s\n", __func__);
	debugfs_remove_recursive(serial->debugfs_dir);
